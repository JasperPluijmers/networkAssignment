...
import fileserver.tui.screens.DownloadScreen;
...
/**
 * ReceiveHandler accepts and writes away packets to a file. It can be made with or without a fileserver.tui, for a fileserver.server
 * or fileserver.client implementation. It puts all packets into a map, it then checks if any of those packets are the next
 * to be written data. If done it checks if the checksum that is received matches its own calculated checksum.
 */
public class ReceiveHandler {
    ...
    public void init() {
        ...
        //Create a new file if the directory is really a folder.
    ...
    public void close() {
        if (active) {
            ...
            try {
                ...
                Logger.log("Can't close output stream");
            }
        }
    }
}
